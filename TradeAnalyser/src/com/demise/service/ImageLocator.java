...
import com.demise.data.MatchResultWrapper;
...
public class ImageLocator implements ChangeListener {
    ...
    List<MatchResultWrapper> vals = new ArrayList<>();
    int xres,yres;

    public void scan( String image, String template, String maskPath ) {
        img = Imgcodecs.imread(image, Imgcodecs.IMREAD_COLOR);
        templ = Imgcodecs.imread(template, Imgcodecs.IMREAD_COLOR);
        mask = Imgcodecs.imread(maskPath, Imgcodecs.IMREAD_COLOR);
        Size sz = new Size(75, 75);
        //Imgproc.resize(templ, templ, sz);
        //Imgproc.resize(mask, mask, sz);
